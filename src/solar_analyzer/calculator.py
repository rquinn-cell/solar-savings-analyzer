...
class SolarSavingsCalculator:
    def __init__(self, bill: XcelSolarBill):
        ...
        cepr_fs = self.bill.cepr_fs_kwh * self.RATE_CEPR_FS
        self.multiplier = ((self.bill.total_electric_due - Decimal("7.10")) / (on_pk + off_pk + cepr_fs) 
                           if (on_pk + off_pk + cepr_fs) > Decimal("0") else Decimal("1.0" ))
